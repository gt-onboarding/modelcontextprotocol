...
El Protocolo de Contexto del Modelo (MCP) admite la paginación de operaciones de listado que pueden devolver conjuntos de resultados grandes. La paginación permite que los servidores entreguen resultados en lotes más pequeños en lugar de todo de una vez.

La paginación es especialmente importante al conectarse a servicios externos por internet, pero también resulta útil en integraciones locales para evitar problemas de rendimiento con conjuntos de datos voluminosos.
...
- El **cursor** es un token de texto opaco que representa una posición en el conjunto de resultados
...
Tras recibir un cursor, el cliente puede _continuar_ con la paginación enviando una solicitud
...
1. Los servidores **DEBERÍAN**:
   ...
   - Manejar cursores no válidos de forma adecuada
