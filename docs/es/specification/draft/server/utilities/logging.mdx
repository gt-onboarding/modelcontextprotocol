...
mensajes de registro estructurados a los clientes. Los clientes pueden controlar la verbosidad del registro configurando
niveles mínimos de registro; los servidores envían notificaciones que incluyen niveles de gravedad,
...
Las implementaciones pueden exponer el registro mediante cualquier patrón de interfaz que se ajuste a sus necesidades&mdash;el protocolo en sí no impone ningún modelo específico de interacción con el usuario.

<div id="capabilities">
  ## Capacidades
</div>

Los servidores que emiten notificaciones de mensajes de registro **DEBEN** declarar la capacidad `logging`:
...
| error     | Condiciones de error             | Fallas en la operación     |
| critical  | Condiciones críticas             | Fallas de componentes del sistema |
| alert     | Debe actuarse de inmediato       | Detección de corrupción de datos |
| emergency | El sistema es inutilizable       | Falla total del sistema    |
...
sequenceDiagram
    ...
    Note over Client,Server: Configurar registro
...
1. Los servidores **DEBERÍAN**:
   ...
   - Usar nombres de logger coherentes
   - Eliminar información confidencial

2. Los clientes **PUEDEN**:
   - Mostrar los mensajes de registro en la interfaz
   - Implementar filtrado y búsqueda de registros
   - Indicar visualmente la severidad
   - Persistir los mensajes de registro
