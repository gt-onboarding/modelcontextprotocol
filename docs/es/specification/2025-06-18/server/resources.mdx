...
El Protocolo de Contexto de Modelo (MCP) proporciona una manera estandarizada para que los servidores expongan
...
Los Recursos en MCP están diseñados para ser **impulsados por la aplicación**, con las aplicaciones anfitrionas
determinando cómo incorporar contexto según sus necesidades.
...
[plantillas de URI](https://datatracker.ietf.org/doc/html/rfc6570). Los argumentos se pueden
completar automáticamente mediante [la API de autocompletado](/es/specification/2025-06-18/server/utilities/completion).
...
Cuando cambie la lista de Recursos disponibles, los Servidores que hayan declarado la capacidad `listChanged`
**DEBERÍAN** enviar una notificación:
...
El protocolo admite suscripciones opcionales a cambios en recursos. Los clientes pueden suscribirse
...
* Mostrar las fechas de modificación u ordenar por recencia
...
Los servidores **DEBERÍAN** usar este esquema solo cuando el cliente pueda obtener y cargar el recurso directamente desde la web por sí mismo; es decir, cuando no necesite leer el recurso a través del Servidor MCP.

Para otros casos de uso, los servidores **DEBERÍAN** preferir otro esquema de URI o definir uno personalizado, incluso si el propio servidor va a descargar el contenido del recurso por internet.

<div id="file">
  ### file://
</div>

Se utiliza para identificar recursos que se comportan como un sistema de archivos. Sin embargo, no es necesario que los recursos correspondan a un sistema de archivos físico real.

Los servidores MCP **PUEDEN** identificar recursos file:// con un
[tipo MIME de XDG](https://specifications.freedesktop.org/shared-mime-info-spec/0.14/ar01s02.html#id-1.3.14),
...
teniendo en cuenta la orientación anterior.
