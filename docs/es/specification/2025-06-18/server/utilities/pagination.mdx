...
El Protocolo de Contexto de Modelo (MCP) admite la paginación de operaciones de listado que pueden devolver conjuntos de resultados grandes. La paginación permite que los servidores entreguen resultados en bloques más pequeños en lugar de todos a la vez.

La paginación es especialmente importante al conectarse a servicios externos a través de internet, pero también resulta útil para integraciones locales, para evitar problemas de rendimiento con conjuntos de datos grandes.

<div id="pagination-model">
  ## Modelo de paginación
</div>

La paginación en MCP utiliza un enfoque basado en cursor opaco, en lugar de páginas numeradas.
...
Los cursores no válidos DEBERÍAN producir un error con el código -32602 (parámetros inválidos).