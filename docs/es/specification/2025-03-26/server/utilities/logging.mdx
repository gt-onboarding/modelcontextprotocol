...
El Protocolo de Contexto del Modelo (MCP) ofrece una manera estandarizada para que los servidores envíen
mensajes de registro estructurados a los clientes. Los clientes pueden controlar la verbosidad del registro estableciendo
niveles mínimos de log, mientras que los servidores envían notificaciones que incluyen niveles de gravedad,
nombres de logger opcionales y datos arbitrarios serializables en JSON.

<div id="user-interaction-model">
  ## Modelo de interacción con el usuario
</div>

Las implementaciones pueden exponer el registro mediante cualquier patrón de interfaz que se adapte a sus necesidades; el propio protocolo no impone ningún modelo específico de interacción con el usuario.
...
El protocolo sigue los niveles de severidad estándar de syslog especificados en
...
Los servidores envían mensajes de registro mediante las notificaciones `notifications/message`:
...
sequenceDiagram
    ...
    Server-->>Client: Resultado vacío
    Note over Server: Solo envía el nivel de error<br/>y superiores
```
...
2. Los clientes **PUEDEN**:
   ...
   - Indicar visualmente el nivel de severidad
