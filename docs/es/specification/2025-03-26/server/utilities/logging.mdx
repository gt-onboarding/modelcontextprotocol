...
El Protocolo de Contexto de Modelo (MCP) ofrece una forma estandarizada para que los servidores envíen mensajes de registro estructurados a los clientes. Los clientes pueden controlar el nivel de detalle del registro estableciendo niveles mínimos, mientras que los servidores envían notificaciones que incluyen niveles de gravedad, nombres de registrador opcionales y datos arbitrarios serializables en JSON.
...
Los servidores que emitan notificaciones de registros **DEBEN** declarar la capacidad `logging`:
...
| alert     | Se debe actuar de inmediato       | Se detectó corrupción de datos |
...
1. Los servidores **DEBERÍAN**:
   - Aplicar límites de velocidad a los mensajes de registro
   - Incluir el contexto relevante en el campo de datos
   - Usar nombres de registrador consistentes
