...
El Protocolo de Contexto de Modelo (MCP) ofrece una forma estandarizada para que los servidores envíen mensajes de registro estructurados a los clientes. Los clientes pueden controlar la verbosidad del registro estableciendo niveles mínimos de registro, y los servidores envían notificaciones que incluyen el nivel de severidad, un nombre de registrador opcional y datos arbitrarios serializables en JSON.

<div id="user-interaction-model">
  ## Modelo de interacción con el usuario
</div>

Las implementaciones pueden exponer el registro a través de cualquier patrón de interfaz que se ajuste a sus necesidades; el protocolo en sí no impone ningún modelo específico de interacción con el usuario.
...
| alert     | Se debe actuar de inmediato      | Corrupción de datos detectada |
...
sequenceDiagram
    ...
    Note over Client,Server: Configurar registro de logs
...
1. Los servidores **DEBERÍAN**:
   ...
   - Usar nombres de registro coherentes
...
1. Los mensajes de registro **NO DEBEN** incluir:
