...
Los Recursos en MCP están diseñados para ser **dirigidos por la aplicación**, y son las aplicaciones host
las que determinan cómo incorporar contexto según sus necesidades.

Por ejemplo, las aplicaciones podrían:

* Exponer recursos mediante elementos de la interfaz para selección explícita, en una vista de árbol o de lista
...
se ajuste a sus necesidades; el protocolo en sí no impone ningún modelo específico de interacción con el usuario.
...
Para obtener el contenido de recursos, los clientes envían una solicitud `resources/read`:
...
a recursos específicos y recibir notificaciones cuando cambien:
...
El protocolo define varios esquemas de URI estándar. Esta lista no es exhaustiva: las implementaciones siempre pueden usar esquemas de URI adicionales y personalizados.
...
Los servidores **DEBERÍAN** usar este esquema solo cuando el cliente pueda obtener y cargar
el recurso directamente desde la web por su cuenta; es decir, cuando no necesite leer el recurso
a través del Servidor de MCP.

Para otros casos de uso, los servidores **DEBERÍAN** preferir otro esquema de URI o definir uno
personalizado, incluso si el propio servidor va a descargar el contenido del recurso de
internet.
...
Los Servidores de MCP **PUEDEN** identificar recursos file:// con un
...
2. **DEBERÍAN** implementarse controles de acceso para recursos sensibles
3. Los datos binarios **DEBEN** estar correctamente codificados
4. Los permisos de los recursos **DEBERÍAN** verificarse antes de realizar operaciones