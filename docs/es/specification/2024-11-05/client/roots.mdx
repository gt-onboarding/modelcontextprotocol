...
El Model Context Protocol (MCP) proporciona una forma estandarizada para que los clientes expongan
“raíces” del sistema de archivos a los servidores. Las raíces delimitan el ámbito en el que los servidores pueden operar
dentro del sistema de archivos, permitiéndoles comprender a qué directorios y archivos tienen
...
se ajuste a sus necesidades: el protocolo en sí no impone ningún modelo de
interacción con el usuario específico.
...
`listChanged` indica si el cliente emitirá notificaciones cuando cambie la lista de Raíces.
...
Cuando las Raíces cambien, los clientes que admitan `listChanged` **DEBEN** enviar una notificación:
...
2. Los servidores **DEBERÍAN**:
   * Manejar casos en los que las Raíces se vuelvan inaccesibles
...
2. Los servidores **DEBERÍAN**:
   * Comprobar la capacidad de Raíces antes de usarlas
   * Manejar de forma adecuada los cambios en la lista de Raíces
   * Respetar los límites de las Raíces en las operaciones
   * Almacenar en caché la información de las Raíces de forma apropiada