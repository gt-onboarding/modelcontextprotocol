...
Le Protocole de contexte de modèle (MCP) offre un moyen standardisé permettant aux serveurs d’exposer des
ressources aux clients. Les ressources permettent aux serveurs de partager des données qui apportent du contexte aux
...
renseignés automatiquement via [l’API de complétion](/fr/specification/2024-11-05/server/utilities/completion).
...
Lorsque la liste des ressources disponibles change, les serveurs qui ont déclaré la fonctionnalité `listChanged`
...
Le protocole prend en charge des abonnements facultatifs aux modifications de Ressources. Les clients peuvent s’abonner à des Ressources spécifiques et recevoir des notifications lorsqu’elles sont modifiées :
...
Les serveurs **DEVRAIENT** n’utiliser ce schéma que lorsque le client est capable de récupérer et de charger la ressource directement depuis le Web par lui-même — c’est-à-dire qu’il n’a pas besoin de lire la ressource via le Serveur MCP.
...
Utilisé pour identifier des ressources qui se comportent comme un système de fichiers. Cependant, ces ressources n’ont pas besoin de correspondre à un système de fichiers physique.
...
de type MIME standard par ailleurs.

<div id="git">
  ### git://
</div>

Intégration au système de contrôle de versions Git.
