...
fonctionnalités d’IA dans diverses applications tout en maintenant des limites de sécurité claires et en
séparant les responsabilités. Fondé sur JSON-RPC 2.0, MCP fournit un protocole de session à état, centré
...
Le MCP repose sur plusieurs principes de conception clés qui guident son architecture et
son implémentation :
...
4. **Les fonctionnalités peuvent être ajoutées progressivement aux serveurs et aux clients**
   * Le protocole de base fournit le strict nécessaire
...
Le MCP définit trois types de messages principaux, fondés sur
[JSON-RPC 2.0](https://www.jsonrpc.org/specification) :

* **Requêtes** : messages bidirectionnels avec une méthode et des paramètres, attendant une réponse
* **Réponses** : résultats réussis ou erreurs correspondant à des identifiants de requête spécifiques
* **Notifications** : messages unidirectionnels ne nécessitant aucune réponse

Chaque type de message respecte la spécification JSON-RPC 2.0 en matière de structure et de
...
Le Protocole de contexte de modèle (MCP) utilise un système de négociation basé sur les capacités, dans lequel les clients et les serveurs déclarent explicitement leurs fonctionnalités prises en charge lors de l’initialisation. Les capacités déterminent quelles fonctionnalités et primitives du protocole sont disponibles pendant une session.
...
* Les [fonctionnalités du serveur](/fr/specification/2024-11-05/server) implémentées doivent être annoncées dans les capacités du serveur
...
* L’[échantillonnage](/fr/specification/2024-11-05/client) nécessite que le client déclare sa prise en charge dans ses capacités
