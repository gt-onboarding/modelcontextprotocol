...
fonctionnalités d’IA dans leurs applications tout en maintenant des frontières de sécurité claires et en
séparant les préoccupations. Reposant sur JSON-RPC 2.0, MCP fournit un protocole de session avec état, centré
...
* Fait appliquer les stratégies de sécurité et les exigences de consentement
* Gère les décisions d’autorisation des utilisateurs
* Coordonne l’intégration avec l’IA/les LLM et l’échantillonnage
...
Une application hôte crée et gère plusieurs clients, chaque client ayant une relation 1:1
...
* Demandent l’Échantillonnage via les interfaces client
...
1. **Les serveurs doivent être extrêmement faciles à créer**
   ...
   * Une séparation claire permet un code facile à maintenir

2. **Les serveurs doivent être hautement composables**
   * Chaque serveur fournit une fonctionnalité ciblée, de façon isolée
...
3. **Les serveurs ne doivent ni lire l’intégralité de la conversation, ni “regarder à l’intérieur” d’autres
   serveurs**
   * Les serveurs ne reçoivent que les informations contextuelles nécessaires
   * L’historique complet de la conversation reste chez l’hôte
...
* Les [fonctionnalités du serveur](/fr/specification/draft/server) implémentées doivent être annoncées dans les capacités du serveur
...
* L’[Échantillonnage](/fr/specification/draft/client) exige que le client déclare sa prise en charge dans ses capacités
