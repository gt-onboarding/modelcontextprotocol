...
Le Protocole de contexte de modèle (MCP) propose une méthode standardisée permettant aux serveurs d’exposer des ressources aux clients. Les ressources permettent aux serveurs de partager des données qui fournissent du contexte aux modèles de langage, comme des fichiers, des schémas de bases de données ou des informations spécifiques à une application. Chaque ressource est identifiée de manière unique par un [URI](https://datatracker.ietf.org/doc/html/rfc3986).
...
* Mettre en place une inclusion automatique du contexte, fondée sur des heuristiques ou la sélection du modèle d’IA

![Exemple de sélecteur de contexte de ressource](/specification/2025-03-26/server/resource-picker.png)
...
* `subscribe` : indique si le client peut s’abonner pour être informé des changements apportés à des
...
aucune, l’une ou l’autre, ou les deux :

```json
{
  "capabilities": {
    "resources": {} // Aucune fonctionnalité n’est prise en charge
  }
}
```
...
Le protocole prend en charge des abonnements optionnels aux modifications de ressources. Les clients peuvent s’abonner
...
Les serveurs **DEVRAIENT** n’utiliser ce schéma que lorsque le client est en mesure de récupérer et de charger la ressource directement depuis le Web, de son côté — c’est-à-dire sans avoir besoin de la lire via le serveur MCP.

Pour les autres cas d’usage, les serveurs **DEVRAIENT** privilégier un autre schéma d’URI, ou en définir un personnalisé, même si le serveur télécharge lui-même le contenu de la ressource sur Internet.

<div id="file">
  ### file://
</div>

Utilisé pour identifier des ressources qui se comportent comme un système de fichiers. Cependant, les ressources n’ont pas besoin de correspondre à un système de fichiers physique réel.
...
comme `inode/directory`, pour représenter des fichiers non réguliers (tels que des répertoires) qui n’ont pas
sinon de type MIME standard.
