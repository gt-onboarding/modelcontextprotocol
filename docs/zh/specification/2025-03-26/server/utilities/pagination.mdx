...
模型上下文协议（MCP）支持为可能返回大型结果集的列表操作提供分页。通过分页，服务器可以分批返回较小的结果块，而非一次性全部返回。

在通过互联网连接外部服务时，分页尤为重要；对于本地集成，分页也有助于避免在处理大型数据集时出现性能问题。
