...
Le Protocole de contexte de modèle (MCP) inclut un mécanisme de ping facultatif qui permet à l’une ou l’autre des parties de vérifier que son homologue est toujours réactif et que la connexion est active.

<div id="overview">
  ## Aperçu
</div>

La fonctionnalité de ping est implémentée selon un modèle simple requête/réponse. Le client ou le serveur peut lancer un ping en envoyant une requête `ping`.
...
sequenceDiagram
    ...
    Expéditeur->>Récepteur: requête ping
...
- Les pings excessifs **DEVRAIENT** être évités pour réduire la surcharge du réseau
