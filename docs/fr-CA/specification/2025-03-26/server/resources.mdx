...
Les ressources dans MCP sont conçues pour être **pilotées par l’application**, les applications hôtes
déterminant comment intégrer le contexte selon leurs besoins.

Par exemple, les applications pourraient :

* Exposer des ressources au moyen d’éléments d’interface pour une sélection explicite, dans une vue en arborescence ou en liste
...
Cependant, les implémentations sont libres d’exposer des ressources via tout modèle d’interface qui
...
ni l’une ni l’autre, l’une seulement, ou les deux :
...
Pour découvrir les ressources disponibles, les clients envoient une requête `resources/list`. Cette opération prend en charge la [pagination](/fr-CA/specification/2025-03-26/server/utilities/pagination).
...
Pour récupérer le contenu des ressources, les clients envoient une requête `resources/read` :
...
complétés automatiquement au moyen de [l’API de complétion](/fr-CA/specification/2025-03-26/server/utilities/completion).
...
<div id="list-changed-notification">
  ### Avis de modification de la liste
</div>
...
Le protocole prend en charge des abonnements facultatifs aux modifications de ressources. Les clients peuvent s’abonner à des ressources précises et recevoir des notifications lorsqu’elles sont modifiées :
...
* `name`: Nom convivial (lisible par l’humain)
...
Les serveurs **DEVRAIENT** utiliser ce schéma uniquement lorsque le client peut récupérer et charger la ressource directement depuis le Web par lui-même — c’est-à-dire qu’il n’a pas besoin de lire la ressource via le Serveur MCP.
