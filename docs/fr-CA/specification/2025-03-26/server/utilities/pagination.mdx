...
Le Protocole de contexte de modèle (MCP) prend en charge la pagination des opérations de liste qui peuvent renvoyer de grands ensembles de résultats. La pagination permet aux serveurs de livrer les résultats en plus petits fragments plutôt que tous d’un coup.
...
La pagination dans MCP utilise une approche opaque basée sur un curseur, plutôt que sur des pages numérotées.
...
- Un champ `nextCursor` facultatif si d’autres résultats sont disponibles
...
3. Les clients **DOIVENT** traiter les curseurs comme des jetons opaques :
   ...
   - Ne pas tenter d’interpréter ou de modifier les curseurs
...
Les curseurs non valides **DEVRAIENT** entraîner une erreur avec le code -32602 (paramètres non valides).